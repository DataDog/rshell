...
> **Linux is the primary target; macOS and Windows are not prioritised for now.** The guidance below remains good practice, but reviewers SHOULD NOT block PRs on hypothetical macOS or Windows breakage. Real bugs on those platforms can be triaged when reported.
