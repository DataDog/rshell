...
func printHelp(callCtx *builtins.CallContext, fs *builtins.FlagSet) {
	...
	callCtx.Out("  - Main-input records, per-record rule evaluations, executed statements, explicit loop iterations, and user-function calls each have a 1,048,576-operation limit per awk run; function depth is capped at 256.\n")
