...
// rmRun runs script with AllowedPaths restricted to dir as read-write and
// remediation mode enabled. Use this wrapper for tests that exercise the
// happy path.
func rmRun(t *testing.T, script, dir string) (string, string, int) {
	t.Helper()
...
// assertGone fails the test if path still exists on disk.
func assertGone(t *testing.T, path string) {
	t.Helper()
	if _, err := os.Lstat(path); !os.IsNotExist(err) {
		t.Fatalf("expected %s to be removed, lstat err=%v", path, err)
	}
}

// assertExists fails the test if path does not exist on disk.
