...
func TestAwkCommandPipes(t *testing.T) {
	...
	stdout, stderr, code = cmdRun(t, `awk 'BEGIN { print "b" | "cat"; print "a"; close("cat") }'`, dir)
	assert.Equal(t, 0, code)
	assert.Equal(t, "", stderr)
	assert.Equal(t, "a\nb\n", stdout)

	stdout, stderr, code = cmdRun(t, `awk 'BEGIN { cmd = "cat"; print "b" | cmd; print "a"; close(cmd) }'`, dir)
	assert.Equal(t, 0, code)
	assert.Equal(t, "", stderr)
	assert.Equal(t, "a\nb\n", stdout)

	stdout, stderr, code = cmdRun(t, `awk 'BEGIN { print "x" | "cat"; print "z"; print "y" | "cat" }'`, dir)
	assert.Equal(t, 0, code)
	assert.Equal(t, "", stderr)
	assert.Equal(t, "x\ny\nz\n", stdout)
	...
	stdout, stderr, code = cmdRun(t, `awk 'BEGIN { print "b" | "sort"; print "mid"; print "a" | "sort"; close("sort") }'`, dir)
	assert.Equal(t, 0, code)
	assert.Equal(t, "", stderr)
	assert.Equal(t, "mid\na\nb\n", stdout)
	...
	stdout, stderr, code = cmdRun(t, `printf '1\n2\n' | awk '{ print $0 | "cat"; print "x" } END { close("cat") }'`, dir)
	assert.Equal(t, 0, code)
	assert.Equal(t, "", stderr)
	assert.Equal(t, "x\nx\n1\n2\n", stdout)

	stdout, stderr, code = cmdRun(t, `awk 'function f(x) { print x | "sort"; print "s" } BEGIN { f("b"); f("a"); close("sort") }'`, dir)
	assert.Equal(t, 0, code)
	assert.Equal(t, "", stderr)
	assert.Equal(t, "s\ns\na\nb\n", stdout)
