...
// not-allowed commands, and the configured AllowedPaths sandbox root specs (or
// a notice when none are configured). When --all is given, disabled commands
...
// printAllowedPaths writes the configured AllowedPaths sandbox root specs, one
// per line. An empty list means no allowed paths are configured, which blocks
// all user-controllable filesystem access — surface that explicitly so
// operators can tell the difference from "no information available". A few
// builtins (ss, ip route, df, ps) intentionally read kernel-state paths outside
// the sandbox and are unaffected by this configuration.
