...
// Each path may be a directory or an individual file. Directories grant access
// to all files within them; individual files grant access to only that file
// (not its parent directory or siblings). When set, only files reachable
// through these paths can be opened, read, or executed.
//
