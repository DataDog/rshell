	...
	case *syntax.CallExpr:
		...
		defer func() {
			for _, restore := range restores {
				...
				//
				// CONTRACT: this guard fires for ANY builtin returning a
				// non-empty Result.NewWorkDir — currently only "cd".
				// A future builtin that (a) accepts an inline PWD/OLDPWD
				// assignment AND (b) returns NewWorkDir for a different
				// purpose would have its inline restore silently suppressed.
