...
// Within each command's slice, symbols are ordered from least safe (top) to
// most safe (bottom), following the same category taxonomy as builtinAllowedSymbols.
var builtinPerCommandSymbols = map[string][]string{
	...
	"ip": {
		"net.FlagBroadcast",    // interface flag: broadcast capability; pure constant, no network connections.
		"net.FlagLoopback",     // interface flag: is loopback; pure constant, no network connections.
		"net.FlagMulticast",    // interface flag: multicast capability; pure constant, no network connections.
	...
	"ping": {
		"net.DefaultResolver",                                 // default system DNS resolver; network I/O is the explicit purpose of this builtin.
	...
	"ss": {
		...
		"syscall.ENOENT",                  // errno constant for "no such file or directory"; distinguishes IPv6-unavailable from genuine errors.
		"os.O_RDONLY",                     // read-only file flag constant; cannot open files by itself.
		"context.Context",                 // deadline/cancellation plumbing; pure interface, no side effects.
