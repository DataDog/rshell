...
var builtinPerCommandSymbols = map[string][]string{
	...
	"tcpdump": {
		...
		"fmt.Errorf",      // 🟢 error formatting; pure function, no I/O.
		"fmt.Sprintf",     // 🟢 string formatting; pure function, no I/O.
		"github.com/gopacket/gopacket.CaptureInfo",                   // 🟢 packet timestamp and length struct from pcap reader; pure data type, no I/O.
		"github.com/gopacket/gopacket.Default",                       // 🟢 default decode options var (no lazy/no-copy); pure constant-like value, no I/O.
		"github.com/gopacket/gopacket.NewPacket",                     // 🟢 decodes raw packet bytes into layers; pure in-memory operation, no network I/O.
		"github.com/gopacket/gopacket.Packet",                        // 🟢 decoded packet interface; no I/O side effects.
		"github.com/gopacket/gopacket/layers.Ethernet",               // 🟢 Ethernet frame struct; pure data type for packet decoding, no I/O.
		"github.com/gopacket/gopacket/layers.ICMPv4",                 // 🟢 ICMPv4 packet struct; pure data type for packet decoding, no I/O.
		"github.com/gopacket/gopacket/layers.ICMPv6",                 // 🟢 ICMPv6 packet struct; pure data type for packet decoding, no I/O.
		"github.com/gopacket/gopacket/layers.IPv4",                   // 🟢 IPv4 header struct; pure data type for packet decoding, no I/O.
		"github.com/gopacket/gopacket/layers.IPv4DontFragment",       // 🟢 IPv4 DF flag constant; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.IPv4MoreFragments",      // 🟢 IPv4 MF flag constant; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.IPv6",                   // 🟢 IPv6 header struct; pure data type for packet decoding, no I/O.
		"github.com/gopacket/gopacket/layers.LayerTypeEthernet",      // 🟢 Ethernet layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.LayerTypeICMPv4",        // 🟢 ICMPv4 layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.LayerTypeICMPv6",        // 🟢 ICMPv6 layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.LayerTypeIPv4",          // 🟢 IPv4 layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.LayerTypeIPv6",          // 🟢 IPv6 layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.LayerTypeTCP",           // 🟢 TCP layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.LayerTypeUDP",           // 🟢 UDP layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.LinkType",               // 🟢 link-layer type identifier returned by pcapgo; pure integer type, no I/O.
		"github.com/gopacket/gopacket/layers.LinkTypeEthernet",       // 🟢 pcap link-layer type constant for Ethernet; returned by live capture handles; pure constant, no I/O.
		"github.com/gopacket/gopacket/layers.TCP",                    // 🟢 TCP segment struct; pure data type for packet decoding, no I/O.
		"github.com/gopacket/gopacket/layers.UDP",                    // 🟢 UDP datagram struct; pure data type for packet decoding, no I/O.
		"github.com/gopacket/gopacket/pcapgo.DefaultNgReaderOptions", // 🟢 default options for pcapng reader; pure constant-like value, no I/O.
		"github.com/gopacket/gopacket/pcapgo.NewNgReader",            // 🟢 opens a pcapng capture file for reading; reads from io.Reader, no raw socket or filesystem access beyond the provided reader.
		"github.com/gopacket/gopacket/pcapgo.NewReader",              // 🟢 opens a pcap capture file for reading; reads from io.Reader, no raw socket or filesystem access beyond the provided reader.
...
var builtinAllowedSymbols = []string{
	...
	"fmt.Sprintf",         // 🟢 string formatting; pure function, no I/O.
	"github.com/gopacket/gopacket.CaptureInfo",                   // 🟢 packet timestamp and length struct from pcap reader; pure data type, no I/O.
	"github.com/gopacket/gopacket.Default",                       // 🟢 default decode options var; pure constant-like value, no I/O.
	"github.com/gopacket/gopacket.NewPacket",                     // 🟢 decodes raw packet bytes into layers; pure in-memory, no network I/O.
	"github.com/gopacket/gopacket.Packet",                        // 🟢 decoded packet interface; no I/O side effects.
	"github.com/gopacket/gopacket/layers.Ethernet",               // 🟢 Ethernet frame struct; pure data type for packet decoding.
	"github.com/gopacket/gopacket/layers.ICMPv4",                 // 🟢 ICMPv4 packet struct; pure data type for packet decoding.
	"github.com/gopacket/gopacket/layers.ICMPv6",                 // 🟢 ICMPv6 packet struct; pure data type for packet decoding.
	"github.com/gopacket/gopacket/layers.IPv4",                   // 🟢 IPv4 header struct; pure data type for packet decoding.
	"github.com/gopacket/gopacket/layers.IPv4DontFragment",       // 🟢 IPv4 DF flag constant; pure constant.
	"github.com/gopacket/gopacket/layers.IPv4MoreFragments",      // 🟢 IPv4 MF flag constant; pure constant.
	"github.com/gopacket/gopacket/layers.IPv6",                   // 🟢 IPv6 header struct; pure data type for packet decoding.
	"github.com/gopacket/gopacket/layers.LayerTypeEthernet",      // 🟢 Ethernet layer type constant; pure constant.
	"github.com/gopacket/gopacket/layers.LayerTypeICMPv4",        // 🟢 ICMPv4 layer type constant; pure constant.
	"github.com/gopacket/gopacket/layers.LayerTypeICMPv6",        // 🟢 ICMPv6 layer type constant; pure constant.
	"github.com/gopacket/gopacket/layers.LayerTypeIPv4",          // 🟢 IPv4 layer type constant; pure constant.
	"github.com/gopacket/gopacket/layers.LayerTypeIPv6",          // 🟢 IPv6 layer type constant; pure constant.
	"github.com/gopacket/gopacket/layers.LayerTypeTCP",           // 🟢 TCP layer type constant; pure constant.
	"github.com/gopacket/gopacket/layers.LayerTypeUDP",           // 🟢 UDP layer type constant; pure constant.
	"github.com/gopacket/gopacket/layers.LinkType",               // 🟢 link-layer type identifier; pure integer type.
	"github.com/gopacket/gopacket/layers.LinkTypeEthernet",       // 🟢 pcap link-layer type constant for Ethernet; returned by live capture handles; pure constant.
	"github.com/gopacket/gopacket/layers.TCP",                    // 🟢 TCP segment struct; pure data type for packet decoding.
	"github.com/gopacket/gopacket/layers.UDP",                    // 🟢 UDP datagram struct; pure data type for packet decoding.
	"github.com/gopacket/gopacket/pcapgo.DefaultNgReaderOptions", // 🟢 default pcapng reader options; pure constant-like value.
	"github.com/gopacket/gopacket/pcapgo.NewNgReader",            // 🟢 opens a pcapng file for reading; reads from io.Reader only.
	"github.com/gopacket/gopacket/pcapgo.NewReader",              // 🟢 opens a pcap file for reading; reads from io.Reader only.
