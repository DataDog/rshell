...
// builtinAllowedSymbols is ordered from least safe (top) to most safe (bottom).
// See the package-level doc in symbols_common.go for the full category guide.
var builtinAllowedSymbols = []string{

	// -------------------------------------------------------------------------
	// 1. Network I/O
	// Symbols that initiate real network connections. Highest risk: a misbehaving
	// builtin could exfiltrate data or establish C2 communication.
	// Only permitted for builtins whose explicit purpose is network I/O (ping).
	// -------------------------------------------------------------------------

	"net.DefaultResolver",                                 // default system DNS resolver; context-aware address lookup; network I/O is the explicit purpose of the ping builtin.
	...
	// Symbols that issue syscalls, load native DLLs, or read kernel data
	// structures. Read-only, but they surface sensitive OS-level information.
	// -------------------------------------------------------------------------

	"golang.org/x/sys/unix.SysctlRaw",    // macOS: reads kernel socket tables via sysctl; read-only, no exec, no filesystem.
	"syscall.ByHandleFileInformation",    // Windows: file info struct for extracting nlink; read-only type, no I/O.
	"syscall.EACCES",                     // POSIX errno constant for permission denied; pure constant, no I/O.
	"syscall.EISDIR",                     // errno constant for "is a directory"; pure constant, no I/O.
	"syscall.ENOENT",                     // errno constant for "no such file or directory"; pure constant, no I/O.
	"syscall.EPERM",                      // POSIX errno constant for operation not permitted; pure constant, no I/O.
	"syscall.EPROTONOSUPPORT",            // POSIX errno constant for protocol not supported; pure constant, no I/O.
	"syscall.Errno",                      // error type for system call error numbers; pure type, no I/O.
	"syscall.GetFileInformationByHandle", // Windows: queries file metadata by handle; read-only, no I/O side effects.
	"syscall.Handle",                     // Windows: file handle type; pure type alias, no I/O.
	"syscall.Stat_t",                     // Unix: file stat struct for extracting UID/GID/nlink; read-only type, no I/O.

	// -------------------------------------------------------------------------
	// 3. OS network state
	// Read-only enumeration of the local machine's network interfaces.
	// No connections are established; data stays on the host.
	// -------------------------------------------------------------------------
	...
	// Read-only access to file/directory metadata (mode bits, stat fields).
	// No file contents are read; no writes or execs are possible.
	// -------------------------------------------------------------------------
	...
	// RE2-backed (linear time, no backtracking), so catastrophic backtracking is
	// impossible. Still CPU-intensive for large inputs; review patterns carefully.
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	// 6. Context & time
	// Deadline/cancellation management and wall-clock reads. No I/O or mutation.
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	// 7. I/O interfaces & streaming utilities
	// Reader/Writer/Closer interfaces and in-memory stream adapters.
	// These types do not themselves perform filesystem or network I/O.
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	// 9. Math & collections
	// Pure arithmetic functions/constants and generic slice operations. No I/O.
	// -------------------------------------------------------------------------
	...
	// -------------------------------------------------------------------------
	// 10. Unicode
	// Pure Unicode data tables and rune-classification functions. No I/O.
	// -------------------------------------------------------------------------
	...
	// Pure error construction and string formatting. No I/O.
	// -------------------------------------------------------------------------
