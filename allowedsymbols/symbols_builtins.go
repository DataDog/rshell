...
var builtinPerCommandSymbols = map[string][]string{
	...
	"tcpdump": {
		...
		"fmt.Errorf",                                               // 🟢 error formatting; pure function, no I/O.
		"fmt.Sprintf",                                              // 🟢 string formatting; pure function, no I/O.
		"github.com/google/gopacket.CaptureInfo",                   // 🟢 packet timestamp and length struct from pcap reader; pure data type, no I/O.
		"github.com/google/gopacket.Default",                       // 🟢 default decode options var (no lazy/no-copy); pure constant-like value, no I/O.
		"github.com/google/gopacket.NewPacket",                     // 🟢 decodes raw packet bytes into layers; pure in-memory operation, no network I/O.
		"github.com/google/gopacket.Packet",                        // 🟢 decoded packet interface; no I/O side effects.
		"github.com/google/gopacket/layers.Ethernet",               // 🟢 Ethernet frame struct; pure data type for packet decoding, no I/O.
		"github.com/google/gopacket/layers.ICMPv4",                 // 🟢 ICMPv4 packet struct; pure data type for packet decoding, no I/O.
		"github.com/google/gopacket/layers.ICMPv6",                 // 🟢 ICMPv6 packet struct; pure data type for packet decoding, no I/O.
		"github.com/google/gopacket/layers.IPv4",                   // 🟢 IPv4 header struct; pure data type for packet decoding, no I/O.
		"github.com/google/gopacket/layers.IPv4DontFragment",       // 🟢 IPv4 DF flag constant; pure constant, no I/O.
		"github.com/google/gopacket/layers.IPv4MoreFragments",      // 🟢 IPv4 MF flag constant; pure constant, no I/O.
		"github.com/google/gopacket/layers.IPv6",                   // 🟢 IPv6 header struct; pure data type for packet decoding, no I/O.
		"github.com/google/gopacket/layers.LayerTypeEthernet",      // 🟢 Ethernet layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/google/gopacket/layers.LayerTypeICMPv4",        // 🟢 ICMPv4 layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/google/gopacket/layers.LayerTypeICMPv6",        // 🟢 ICMPv6 layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/google/gopacket/layers.LayerTypeIPv4",          // 🟢 IPv4 layer type constant for packet.Layer() lookup; pure constant, no I/O.
		"github.com/google/gopacket/layers.LayerTypeIPv6",          // 🟢 IPv6 layer type constant for packet.Layer() lookup; pure constant, no I/O.
...
var builtinAllowedSymbols = []string{
	...
	"fmt.Sprintf",                                              // 🟢 string formatting; pure function, no I/O.
	"github.com/google/gopacket.CaptureInfo",                   // 🟢 packet timestamp and length struct from pcap reader; pure data type, no I/O.
	"github.com/google/gopacket.Default",                       // 🟢 default decode options var; pure constant-like value, no I/O.
	"github.com/google/gopacket.NewPacket",                     // 🟢 decodes raw packet bytes into layers; pure in-memory, no network I/O.
	"github.com/google/gopacket.Packet",                        // 🟢 decoded packet interface; no I/O side effects.
	"github.com/google/gopacket/layers.Ethernet",               // 🟢 Ethernet frame struct; pure data type for packet decoding.
	"github.com/google/gopacket/layers.ICMPv4",                 // 🟢 ICMPv4 packet struct; pure data type for packet decoding.
	"github.com/google/gopacket/layers.ICMPv6",                 // 🟢 ICMPv6 packet struct; pure data type for packet decoding.
	"github.com/google/gopacket/layers.IPv4",                   // 🟢 IPv4 header struct; pure data type for packet decoding.
	"github.com/google/gopacket/layers.IPv4DontFragment",       // 🟢 IPv4 DF flag constant; pure constant.
	"github.com/google/gopacket/layers.IPv4MoreFragments",      // 🟢 IPv4 MF flag constant; pure constant.
	"github.com/google/gopacket/layers.IPv6",                   // 🟢 IPv6 header struct; pure data type for packet decoding.
	"github.com/google/gopacket/layers.LayerTypeEthernet",      // 🟢 Ethernet layer type constant; pure constant.
	"github.com/google/gopacket/layers.LayerTypeICMPv4",        // 🟢 ICMPv4 layer type constant; pure constant.
	"github.com/google/gopacket/layers.LayerTypeICMPv6",        // 🟢 ICMPv6 layer type constant; pure constant.
	"github.com/google/gopacket/layers.LayerTypeIPv4",          // 🟢 IPv4 layer type constant; pure constant.
	"github.com/google/gopacket/layers.LayerTypeIPv6",          // 🟢 IPv6 layer type constant; pure constant.
	...
	"github.com/google/gopacket/pcapgo.NewNgReader",            // 🟢 opens a pcapng file for reading; reads from io.Reader only.
	"github.com/google/gopacket/pcapgo.NewReader",              // 🟢 opens a pcap file for reading; reads from io.Reader only.
