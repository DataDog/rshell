...
`systemctl` is available only when the runner uses `interp.WithMode(interp.ModeRemediation)` (CLI: `--mode remediation`). This command-wide gate applies to every invocation, including bare `list-units`, `status`, `show`, the `is-*` predicates, and `--help`; no grant lookup or manager access occurs in read-only mode. Once enabled, `systemctl` provides bounded unit inspection and control without executing the host binary or exposing a generic D-Bus client. A bare invocation is the restricted equivalent of `list-units`. `--system` and `--no-pager` are accepted as no-op compatibility flags because only the configured system manager is available and rshell never starts a pager.
