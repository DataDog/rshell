...
> **Note:** The `ss`, `ip route`, `df`, `free`, and `pmap` builtins bypass `AllowedPaths` for their kernel-state reads. `ss` and `ip route` open `/proc/net/*` paths directly; `df` reads `/proc/self/mountinfo` (Linux) or calls `getfsstat(2)` (macOS), then issues `unix.Statfs(2)` against every kernel-reported mount point; `free` reads `/proc/meminfo` (Linux only) directly via `os.Open`; `pmap` performs bounded direct reads of `<ProcPath>/<pid>/{comm,maps,smaps}` (Linux), calls `VirtualQueryEx` (Windows), or calls `proc_pidinfo(PROC_PIDREGIONINFO)` via the raw `syscall.SYS_PROC_INFO` trap (macOS). These paths are hardcoded — or, for `pmap`, combine a proc root fixed by the embedding application with a numeric PID, never arbitrary script input — and the reads return metadata only (block / inode counts, filesystem type, block size, aggregate memory/swap counters, or per-mapping address ranges and permissions). There is no sandbox-escape risk, but operators cannot use `AllowedPaths` to block `ss` from enumerating local sockets, `ip route` from reading the routing table, `df` from reporting mount-table capacity, `free` from reporting host memory usage, or `pmap` from reporting a process's memory map — these reads succeed regardless of the configured path policy.
