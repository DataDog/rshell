...
- **Filesystem metadata:** `stat -f FILE...` resolves every operand through `AllowedPaths` and queries the filesystem through the resulting handle. It is available on Linux, macOS, and Windows; Windows reports inode totals as unavailable because NTFS and ReFS do not expose a fixed POSIX-style inode pool.
