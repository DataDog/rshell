...
1. **Inventory once.** First root-touching call: `help`, shallow `find` inventory for each root, and small bounded probes only for prompt-named or obvious current/rotated files. Label empty/fallback roots immediately.
2. **Prove once.** Second call: assign literal file variables from inventory. In one labeled script collect cause tokens, impact/status, driver/source fields, alternative/recovery counts, and one correlation or aggregation. Include prompt-named theories and red herrings here.
...
Stop when the ledger has: productive root/file, incident window, affected object, raw cause and impact/status tokens, driver/source if present, recovery/absence, rejected alternatives, historical/rotated matches, and selected zero-count layers.
...
- Prefer inventory first, then literal files:
...
- Every sample `grep` needs `-m` unless it is `grep -c` or a tight aggregation. Avoid large `sed` windows, unbounded `grep | tail`, and full-date regexes until the date prefix is observed.
...
- `Commands run`: one bullet per recorded rshell invocation. Each bullet independently repeats exact `./rshell --allow-all-commands`, literal full `--allowed-paths=...`, and a short purpose. Include exact `-c` only when readable; if omitted, do not quote a fake command.
