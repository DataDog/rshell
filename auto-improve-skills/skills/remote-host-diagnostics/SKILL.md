...
2. **Prove once.** Second call: select likely current files, prompt-named rotated/noise files, and at most two corroborating layers. In one labeled script collect exact cause tokens, impact/status, driver/source fields, scoped alternative/recovery counts, and one correlation or aggregation.
...
- After inventory, every sample `grep` needs `-m` unless it is a `grep -c` count or tight aggregation. Avoid large `sed` windows, unbounded `grep | tail`, and time-only sweeps.
...
- **Agent/telemetry:** Separate config, credential/auth, intake/APM, queue/aggregator/collector/forwarder, sibling health, and recovery. In the proof pass include current and rotated/noise matches for config-validation, auth/status, flush/drop/queue, source/transaction/config, and APM/trace health terms. Pair the cause with delivery impact and reject healthy sibling paths or historical recovered lookalikes with counts or labeled lines.
- **HTTP/service:** Correlate route plus numeric status across access/proxy, service, and dependency/system layers. For dependency exhaustion, collect owner/client/fanout fields such as `application_name=`, worker/job/client names, active/max values, reserved-slot wording, or native dependency errors in the same proof pass. Reject prompt theories, cache/flag/payment/DNS-style red herrings, and rotated recovered bursts with scoped evidence.
...
- **Certificates/container fallback:** Inspect primary and fallback roots together. State which root was empty and which produced evidence. Quote the certificate/x509 clause, compare observed current time with `NotBefore`/`NotAfter` when present, and cite timing, renewal, kubelet/syslog, or equivalent system evidence before choosing timing versus certificate material.
...
- `Commands run`: one bullet per recorded rshell invocation. Repeat exact `./rshell --allow-all-commands`, literal full `--allowed-paths=...`, and `-c`; if the script is long, summarize labels outside the quoted command, but never shorten paths or use ellipses/placeholders.
...
Final check: recorded calls only; literal allowed paths; raw tokens in finding/evidence; actual file names per layer; scoped zero-counts; labeled current, historical, recovered, fallback, and different-source evidence; no unsupported socket/PID, real-host, compromise, placeholder-command, invented-token, or remediation claims.
