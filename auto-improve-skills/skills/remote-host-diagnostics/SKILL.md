...
- Plan a short read-only checklist up front: confirm safety/scope and target, clarify symptoms and timeframe, order checks from cheapest overview to targeted hypothesis tests, and sample only relevant layers (health/resources, process/service/listener state, configuration, recent logs/events); if a probe fails or returns no data, do not repeat it unchanged—use help/error text to choose one narrower documented alternative or note the limitation.
