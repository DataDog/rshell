...
func TestPrivilegedHelperRootIntegration(t *testing.T) {
	...
	target := filepath.Join(dir, "root-only.log")
	require.NoError(t, os.WriteFile(target, []byte("data that must be truncated"), 0o600))
	...
	credential := privilegedhelper.Credential{
		...
		AllowedCommands: []string{"rshell:truncate"}, AllowedPaths: []string{dir + ":rw"},
	...
	nonElevated := signedIntegrationRequest(t, privateKey, "truncate -s 0 "+target, dir)
	...
	elevated := signedIntegrationRequest(t, privateKey, "sudo truncate -s 0 "+target, dir)
...
func signedIntegrationRequest(t *testing.T, privateKey ed25519.PrivateKey, command, allowedDir string) privilegedhelper.ExecuteRequest {
	t.Helper()
	...
	task := &privilegedhelper.PrivateActionTask{
		...
		SystemInputs: &privilegedhelper.SystemInputs{Input: &privilegedhelper.SystemInputs_RemoteAction{
			RemoteAction: &privilegedhelper.RemoteAction{AllowedCommands: []string{"rshell:truncate"}, AllowedPaths: []string{allowedDir + ":rw"}},
		}},
	}
