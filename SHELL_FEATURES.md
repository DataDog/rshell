...
- ✅ `ping [-c N] [-W N] [-i N] HOST` — send ICMP ECHO_REQUEST to network hosts using the pro-bing library; `-c` sets probe count (default 4, max 1000), `-W` sets per-probe timeout in seconds (default 2, max 60), `-i` sets interval between probes (default 1); attempts raw-socket (privileged) mode first and falls back to UDP-based ICMP on Linux/macOS; on Windows, privileged mode is used (works without elevation on Windows 10+)
