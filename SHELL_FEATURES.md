...
- ✅ `jq [-hcernsR] [--arg NAME VALUE] [--argjson NAME JSON] [FILTER] [FILE]...` — native JSON filtering without a host binary or third-party jq engine, targeting jq 1.8 semantics within the supported surface and documented divergences; supports identity, object fields, array/object indexes and iteration, optional `?`, pipes and generators, JSON literals and constructors, variables, comparisons, boolean operators, alternative `//`, arithmetic, and `select`, `map`, `length`, `keys`, `has`, `type`, `not`, and `empty`; an omitted FILTER means identity, as in jq 1.7.1 and later; at most 64 file operands are accepted, and each must be an identity-verified regular file opened through `AllowedPaths` (FIFOs, devices, sockets, and descriptor portals are rejected); filters are capped at 64 KiB and 4,096 syntax nodes, cumulative input at 8 MiB, raw lines at 1 MiB, JSON nesting at 64 levels, values at 65,536 nodes and 4 MiB of logical data, evaluation at 100,000 steps and 10,000 results, retained intermediate batches and construction state at 131,072 nodes and 8 MiB across the evaluator, and generated output at 1 MiB; evaluation step and result budgets are cumulative across the invocation, and exhausting either is a hard failure rather than silent truncation; at most 64 `--arg`/`--argjson` bindings and 1 MiB of aggregate binding text are accepted, with referenced parsed values sharing a 65,536-node/4-MiB aggregate bound; integer literals and integer-only arithmetic remain exact within a 256-bit magnitude bound, while decimal/exponent literals, mixed arithmetic, and non-integral division use `float64` with jq-style overflow saturation; a value whose magnitude, node count, nesting, or size breaches a per-value bound is reported and skipped so later inputs still run, and the exit status is 5; assignments, user-defined functions, reduce/foreach, try/catch, regex, recursion, modules, environment access, file-based filters/variables, streaming, dates, formatters, `as` bindings, `if`/`then`/`else`, array and string slices (`.[a:b]`), array-index-by-array (`.[[x]]`), and `$__loc__` are unavailable
...
- ✅ Whole-run execution timeout — callers can bound a `Run()` call via `context.Context`, `interp.MaxExecutionTime`, or the CLI `--timeout` flag; the deadline applies to the entire script, not each individual command; commands check cancellation between bounded operations, but a caller-provided stdin/stdout/stderr implementation that is already blocked inside `Read` or `Write` can delay return because the runner does not close, alter, or asynchronously continue I/O on borrowed streams
...
- **Exact integer arithmetic in `jq`**: upstream jq arithmetic generally passes numbers through its floating-point representation, so operations on large integers can lose precision. The rshell builtin preserves integer literals and integer-only `+`, `-`, `*`, `%`, and exact division as bounded 256-bit integers; decimal/exponent literals, mixed arithmetic, and non-integral division use `float64`. One consequence is that a signed zero does not survive integer arithmetic: `0 * -1` is `0` rather than jq's `-0`, though a `-0` read from input or written as a decimal literal keeps its sign.
- **256-bit integer magnitude bound in `jq`**: upstream jq carries an arbitrary-precision decimal literal through unchanged, so it accepts integers of any length. The rshell builtin caps integer magnitude at 256 bits and reports a value beyond it as a per-value error, skipping that input and continuing with the next. The bound is deliberate rather than incidental: `big.Int` multiplication doubles the operand's bit length, so an unbounded magnitude combined with chained multiplication in a filter is a memory-exhaustion vector.
- **Floating-point rendering in `jq`**: jq 1.8 can retain the source spelling of an untouched decimal/exponent literal, including trailing zeroes. The rshell builtin stores those values as `float64` immediately, so output uses normalized Go JSON-number formatting rather than preserving the input spelling. Overflow saturates to `±1.7976931348623157e+308`, matching what jq prints for a computed infinity; an underflowing literal becomes `0`.
- **Strict JSON input in `jq`**: jq 1.8 accepts some non-standard input numbers with leading zeroes (for example, `01`), while the rshell builtin rejects them as malformed JSON instead of silently normalizing them. Upstream jq can also treat a raw NUL inside a malformed string as an internal buffer terminator and continue that string across later operands; the builtin rejects raw NUL immediately as an unescaped control character instead of reproducing that chunk-dependent parser quirk. Unpaired surrogate escapes follow jq: an unpaired low surrogate (`"\udc00"`) decodes to U+FFFD, while an unpaired high surrogate is a parse error.
