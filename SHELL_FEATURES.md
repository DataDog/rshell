...
- ✅ AllowedCommands — restricts which registered rshell builtins may be executed; commands require the `rshell:` namespace prefix (e.g. `rshell:cat`); if not set, no commands are allowed
...
- ❌ External commands — unavailable through the public API; `AllowedCommands` authorizes registered rshell builtins only
