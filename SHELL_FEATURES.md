...
- ✅ AllowedPaths filesystem sandboxing — restricts all file access (read and write) to specified directories. Entries may end with `:ro` or `:rw` to indicate read-only and read-write permissions, respecitvely; entries without a suffix default to read-only. Cross-root symlink fallback is read-only to avoid TOCTOU on writes
||||||| parent of 6cf4c4dc (Handle Windows path mode suffixes)
