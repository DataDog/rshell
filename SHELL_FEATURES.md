...
- ✅ `systemctl start|stop|restart|reload|status UNIT`; `systemctl show --property=ActiveState --value UNIT` — guarded remediation command; delegates one lifecycle or status action and unit to the host command handler
