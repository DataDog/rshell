...
- ✅ `jq [-hcernsR] [--arg NAME VALUE] [--argjson NAME JSON] FILTER [FILE]...` — native JSON filtering without a host binary or third-party jq engine, targeting jq 1.8 semantics within the supported surface and documented divergences; supports identity, object fields, array/object indexes and iteration, optional `?`, pipes and generators, JSON literals and constructors, variables, comparisons, boolean operators, alternative `//`, arithmetic, and `select`, `map`, `length`, `keys`, `has`, `type`, `not`, and `empty`; at most 64 file operands are accepted, and each must be an identity-verified regular file opened through `AllowedPaths` (FIFOs, devices, sockets, and descriptor portals are rejected); filters are capped at 64 KiB and 4,096 syntax nodes, cumulative input at 8 MiB, raw lines at 1 MiB, JSON nesting at 64 levels, values at 65,536 nodes and 4 MiB of logical data, evaluation at 100,000 steps and 10,000 results, retained intermediate batches and construction state at 131,072 nodes and 8 MiB across the evaluator, and generated output at 1 MiB; at most 64 `--arg`/`--argjson` bindings and 1 MiB of aggregate binding text are accepted, with referenced parsed values sharing a 65,536-node/4-MiB aggregate bound; integer literals and integer-only arithmetic remain exact within a 256-bit magnitude bound, while decimal/exponent literals, mixed arithmetic, and non-integral division use finite `float64`; assignments, user-defined functions, reduce/foreach, try/catch, regex, recursion, modules, environment access, file-based filters/variables, streaming, dates, and formatters are unavailable
...
- **Exact integer arithmetic in `jq`**: upstream jq arithmetic generally passes numbers through its floating-point representation, so operations on large integers can lose precision. The rshell builtin preserves integer literals and integer-only `+`, `-`, `*`, `%`, and exact division as bounded 256-bit integers; decimal/exponent literals, mixed arithmetic, and non-integral division use `float64`.
- **Floating-point rendering in `jq`**: jq 1.8 can retain the source spelling of an untouched decimal/exponent literal, including trailing zeroes. The rshell builtin stores those values as finite `float64` immediately, so output uses normalized Go JSON-number formatting rather than preserving the input spelling.
- **Strict JSON input in `jq`**: jq 1.8 accepts some non-standard input numbers with leading zeroes (for example, `01`), while the rshell builtin rejects them as malformed JSON instead of silently normalizing them. Upstream jq can also treat a raw NUL inside a malformed string as an internal buffer terminator and continue that string across later operands; the builtin rejects raw NUL immediately as an unescaped control character instead of reproducing that chunk-dependent parser quirk.
