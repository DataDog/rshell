...
- ✅ `tcpdump (-r FILE | -i IFACE) [-c N] [-n|-nn] [-v|-vv|-vvv] [-q] [-e] [-A] [-x|-xx] [-X|-XX] [-t|-tt|-ttt|-tttt] [-s N] [FILTER]` — read and display packet captures from pcap/pcapng files (`-r`) or capture live packets from a network interface (`-i`; requires `CAP_NET_RAW` on Linux or root on macOS); filter supports `host`, `src/dst host`, `port`, `src/dst port`, `tcp/udp/icmp/ip/ip6`, `and/or/not`, and parentheses; exactly one of `-r` or `-i` is required; `-w` (file write), `-z` (exec), `-Z` (priv esc), `-C`/`-W`/`-G` (rotation), and `-D` (list interfaces) are rejected
