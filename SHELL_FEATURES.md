...
- **Strict JSON numbers in `jq`**: jq 1.8 accepts some non-standard input numbers with leading zeroes (for example, `01`). The rshell builtin rejects them as malformed JSON instead of silently normalizing them.
