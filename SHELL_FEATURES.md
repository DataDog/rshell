...
- ✅ `rm [-v] FILE...` — remove files; **remediation mode only**, targets must be within a `:rw` `AllowedPaths` root; only regular files and symlinks may be removed (directories are always rejected, even empty ones — there is no recursive or `-d`/`--dir` mode); a symlink argument removes the link itself, never its referent; at most 10 files per invocation, checked before any file is removed; `-v`/`--verbose` prints `removed 'FILE'` per file; `-r`/`-R`/`--recursive`, `-f`/`--force`, `-i`/`-I`/`--interactive`, `-d`/`--dir`, `--preserve-root`, `--no-preserve-root`, and `--one-file-system` are rejected as unknown flags
