...
- ✅ `ping [-c N] [-W N] [-i N] HOST` — send ICMP ECHO_REQUEST to network hosts using the datadog-traceroute library; `-c` sets probe count (default 4, max 1000), `-W` sets timeout in seconds, `-i` sets interval between probes
