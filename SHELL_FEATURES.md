...
- ✅ `setfacl -m g:GROUP:PERMS [-R] [-d] PATH` — add or replace a single named-group entry in PATH's access ACL, or with `-d` its default ACL; **remediation mode only**, and only within a `:rw` `AllowedPaths` root; Linux only (exits 1 with `setfacl: not supported on this platform` on macOS/Windows); GROUP is resolved to a numeric GID via `/etc/group`, bypassing `AllowedPaths` the same way `ss`/`ip route`/`df`/`free` bypass it for other trusted kernel/host metadata reads (see AGENTS.md); PERMS is zero to three letters drawn from `r`, `w`, `x`; `-R`/`--recursive` applies the entry to PATH and, for a directory, every entry beneath it (symlinks are never followed); all ACL reads/writes go through `callCtx.GetACL`/`SetACL`, which resolve PATH the same TOCTOU-safe, no-follow, hard-link-rejecting way `Truncate` does; only a single `g:` entry per invocation is supported — `-x`/`-b`/`--set`/`-M`/`--restore`/`-n`/`-L`/`-P` and `u:`/`o:`/`m:` entries are rejected or out of scope
