...
- ✅ DeniedPaths filesystem deny rules — explicitly deny subtrees inside AllowedPaths. Entries may end with `:r` (deny reads and writes) or `:w` (deny writes only); entries without a suffix default to `:r`. Direct `:r` denied children are filtered from directory listings; `:w` denied children remain visible and readable. Kernel-state builtins that intentionally bypass AllowedPaths (`ss`, `ip route`, `df`, and `ps` proc reads) also bypass DeniedPaths
